...
import { useValue, dbPush, dbSet, SERVER_TS, toList } from '../lib/db'
...
function HostView({ base, meta, players }) {
  ...
  const { assigned, clashGroups, next, pressed, done } = useMemo(() => resolve(tapsRaw, clashMs, N), [tapsRaw, clashMs, N])
  ...
  return (
    <div className="text-center">
      ...
      <div className="mt-4 flex flex-wrap justify-center gap-2 max-w-2xl mx-auto">
        {assigned.map((a) => (
          <span key={a.id} className="clay-inset px-3 py-1.5 font-bold animate-pop">{a.k}. {a.by}</span>
        ))}
          ...
          <div className="mt-2 space-y-1.5 max-w-md mx-auto">
            {clashGroups.map((g, i) => (
              <div key={i} className="clay-inset px-3 py-2 font-display animate-pop" style={{ background: 'var(--c-coral)', color: '#fff' }}>
                💥 {g.map((t) => t.by).join('  ↔  ')} <span className="text-sm opacity-90">({g.length}명 동시)</span>
              </div>
            ))}
          </div>
        </div>
...
function PlayerView({ base, meta, players, me }) {
  ...
  const { assigned, clashGroups, next, done } = useMemo(() => resolve(tapsRaw, clashMs, N), [tapsRaw, clashMs, N])

  const hasPressed = useMemo(() => toList(tapsRaw).some((t) => t.pid === me.id), [tapsRaw, me.id])
  ...
  return (
    <div className="text-center">
      ...
      {myClash ? (
        <p className="mt-3 font-display" style={{ color: 'var(--c-coral)' }}>💥 {myClash.filter((t) => t.pid !== me.id).map((t) => t.by).join(', ') || '누군가'} 랑 동시에 눌러서 걸렸어요! 벌칙 🍺</p>
      ) : hasPressed ? (
