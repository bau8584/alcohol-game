// 금고 털이(은행 강도) — 모두가 같은 타이머를 본다. 시작하면 '털고 있는 시간'이 올라가고(=챙긴 액수),
// 경찰이 언제 급습할지는 아무도 모른다. 튀면 그때까지가 점수. 안 튀고 붙잡히면 0점 + 벌칙.
// 오래 털수록 이득이지만 경찰은 3초~최대초 사이 랜덤 급습. 하나둘 튀는 게 화면에 실시간으로 보인다.
    ...
    <div className="text-center">
      ...
      {idle ? (
        <div className="mt-4">
          ...
          <p className="mt-4 font-display text-xl" style={{ color: 'var(--ink-soft)' }}>오래 털수록 점수 ↑ · 잡히면 0점 🍺</p>
        </div>
      ) : (
        <div className="mt-4">
          ...
          {boomedAt ? (
            ...
            <>
              <div className="mt-1 font-display" style={{ color: elapsed > MIN_MS ? 'var(--c-coral)' : 'var(--ink-soft)' }}>
                {elapsed > MIN_MS ? '🚨 위험 · 언제든 경찰 급습!' : '🟢 아직 조용함'}
              </div>
  ...
  return (
    <div className="text-center">
      ...
      <button onPointerDown={bank} disabled={!active}
        ...
        <div className="font-display text-3xl">{active ? (inDanger ? '🏃 지금 튀어?!' : '오래 털수록 점수 ↑') : ''}</div>
      ...
      {caught ? (
        <p className="mt-3 font-display" style={{ color: 'var(--c-coral)' }}>
          {lateBank ? `경찰 온 뒤에 튀었어요 (${fmt(mine)}초) · 벌칙 🍺` : '안 튀고 있다 붙잡혔어요 · 벌칙 🍺'}
        </p>
      ) : mine != null ? (
        <p className="mt-3 text-sm" style={{ color: 'var(--c-mint)' }}>
          {fmt(mine)}초에 튐 · 현재 {myRank}위 {boomedAt ? '' : '· 더 버틴 사람에게 밀릴 수 있어요'}
        </p>
      ) : active ? (
        <p className="mt-3 font-display" style={{ color: inDanger ? 'var(--c-coral)' : 'var(--ink-soft)' }}>
          {inDanger ? '🚨 언제든 경찰 급습! 욕심내면 0점 🍺' : `${(MIN_MS / 1000).toFixed(0)}초까진 조용 · 그 뒤부턴 경찰 랜덤 급습`}
        </p>
      ) : (
        <p className="mt-3 text-sm" style={{ color: 'var(--ink-soft)' }}>오래 털수록 고득점 · 잡히면 0점 🍺</p>
      )}
    </div>
  )
}
